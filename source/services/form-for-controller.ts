...
class FormForController {
    ...
    fieldDatum.unwatchers.push(
      this.$scope_.$watch('formFor.' + fieldName,
        (newValue:any, oldValue:any) => {
          ...
          fieldDatum.bindableWrapper.pristine =
            !this.$scope_.formForStateHelper.hasFieldBeenModified(bindableFieldName);
        }));
   ...
   */
  registerSubmitButton(submitButtonScope:ng.IScope):SubmitButtonScope {
    var bindableWrapper:SubmitButtonScope = {
      disabled: false
    };
